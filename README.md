...
游戏系统可能会为玩家分配特殊任务，完成任务可以获得额外奖励分数。以下是所有可用的任务类型：

#### 🐺 自刀狼人 (self_kill_werewolf)
...
- **奖励**: 50分
- **适用角色**: 女巫
- **任务描述**: 第一天晚上不能使用毒药或者解药

#### 👤 寡言村民 (silent_villager)

- **奖励**: 50分
- **适用角色**: 村民
- **任务描述**: 发言和遗言都不超过20个字（去除标点符号后）

#### 🧙‍♀️ 解药过敏 (heal_allergy_witch)
...
#### 🐺 亡命鸳鸯 (desperate_lovers)

- **奖励**: 100分
- **适用角色**: 狼人
- **任务描述**: 
  - 如果狼队友死了，下次发言说自己是狼人
  - 如果你死了，遗言承认自己是狼人并说出谁是狼队友
...
- **适用角色**: 村民
- **任务描述**: 
  - 每次发言都说我是警察并指认一个玩家是狼人
  - 每次投票都投中狼人

**注意事项**：

1. 任务信息通过环境变量传递给 Player Agent
2. Agent 需要根据任务要求调整决策逻辑
3. 部分任务需要 AI 检测发言内容，确保发言符合任务要求
4. 任务奖励在游戏结束后结算
